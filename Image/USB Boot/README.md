INTRODUCTION:
This script allows you to run a python program called "program.py" just by copying the file on a USB stick and connecting it to the Raspberry Pi.
...
Copy the test python program in the USB folder to the USB device. Insert the USB device in the Raspberry Pi, wait a few seconds for the program to run. Remove it and insert into the computer. An output.txt file should be there with the program output and an error.txt should be there which would have the errors if any.
