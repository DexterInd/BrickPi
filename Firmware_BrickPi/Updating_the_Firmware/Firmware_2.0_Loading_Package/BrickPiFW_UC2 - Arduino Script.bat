...
set /p port="Enter the COM port (like 'COM5'): " %=%
pause
) else ( 
...
: Write the fuses
avrdude -P %port% -b 19200 -c avrisp -p m328p -U lfuse:w:0xFF:m
pause
avrdude -P %port% -b 19200 -c avrisp -p m328p -U hfuse:w:0xDA:m
pause
avrdude -P %port% -b 19200 -c avrisp -p m328p -U efuse:w:0x05:m
pause
: Write the bootloader hex file.  Assumes it's in the same directory.
avrdude -P %port% -b 19200 -c avrisp -p m328p -U flash:w:ATmegaBOOT_168_atmega328.hex
pause

: Write the compiled BrickPi file.  Assumes it's in the same directory.
avrdude -P %port% -b 19200 -c avrisp -p m328p -U flash:w:BrickPiFW_Compressed_Communication.cpp.hex
pause
...
avrdude -P %port% -b 19200 -c avrisp -p m328p -U eeprom:w:BrickPi_EEPROM_UC2.hex
